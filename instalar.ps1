...
if (Test-Path $PythonwExe) {
    ...
    # Detener procesos huérfanos anteriores de auto_vigilante si existieran
    Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like "*auto_vigilante.py*" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }
    Start-Process -FilePath $PythonwExe -ArgumentList "`"$VigilanteScript`" `"$ProjectRoot`"" -WindowStyle Hidden
}

# 4. Generación inicial rápida del catálogo TEMARIO_ACTIVO.md
if (Test-Path $PythonExe) {
    Write-Host "[4/4] Sincronizando catálogo TEMARIO_ACTIVO.md en la raíz del proyecto..."
    & $PythonExe "$GestorScript" "$ProjectRoot"
}
